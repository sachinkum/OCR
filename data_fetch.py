...
class OcrData():
        ...
        for mfile in mfiles:
            ...
            for index, line in enumerate(lines):
                if line.startswith('list.ALLnames'):
                    ...
                    end_index = index - 1
            if 'Img' in mfile:
                self.images += [os.path.join(*(['Englishimg', 'Img'] + line.strip()[1:-1].split('/'))) for line in
                                lines[start_index + 1:end_index]]
            elif 'Fnt' in mfile:
                self.images += [os.path.join(*(['EnglishFnt', 'Fnt'] + line.strip()[1:-1].split('/'))) for line in
                                lines[start_index + 1:end_index]]
            elif 'Hnd' in mfile:
                self.images += [os.path.join(*(['EnglishHnd', 'Hnd'] + line.strip()[1:-1].split('/'))) for line in
                                lines[start_index + 1:end_index]]
            m.close()
    ...
    def generate_best_hog_model(self):
        ...
        clf = Pipeline(
            [('hog', HOGFeatures(orientations=10, pixels_per_cell=(5, 5), cells_per_block=(2, 2), size=self.img_size)),
             ('clf', LinearSVC(C=2.0))])
